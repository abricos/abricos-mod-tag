...
class Tag {
    ...
    public function __construct(TagManager $manager){
        ...
        $models->RegisterClass('UserRole', 'TagUserRole');
        $models->RegisterClass('UserRoleList', 'TagUserRoleList');
        $models->RegisterClass('Group', 'TagGroup');
        $models->RegisterClass('GroupList', 'TagGroupList');
        $models->RegisterClass('Account', 'TagAccount');
        $models->RegisterClass('AccountList', 'TagAccountList');
        $models->RegisterClass('User', 'TagUser');
        $models->RegisterClass('UserList', 'TagUserList');
        $models->RegisterClass('Category', 'TagCategory');
        $models->RegisterClass('CategoryList', 'TagCategoryList');
        $models->RegisterClass('Oper', 'TagOper');
        $models->RegisterClass('OperList', 'TagOperList');
        $models->RegisterClass('OperMove', 'TagOperMove');
        $models->RegisterClass('OperMoveList', 'TagOperMoveList');
        $models->RegisterClass('Balance', 'TagBalance');
        $models->RegisterClass('BalanceList', 'TagBalanceList');
    }
    ...
    public function AccountList(){
        ...
        $rows = TagQuery::AccountList($this->db, Abricos::$user->id);
        /** @var TagAccountList $list */
        ...
        $rows = TagQuery::AUserRoleListByAId($this->db, $accountIds);
        while (($d = $this->db->fetch_array($rows))){
    ...
    public function GroupList(){
        ...
        /** @var TagGroupList $list */
        $list = $this->models->InstanceClass('GroupList');
        $rows = TagQuery::GroupListByIds($this->db, $groupIds, Abricos::$user->id);
        while (($d = $this->db->fetch_array($rows))){
            $list->Add($this->models->InstanceClass('Group', $d));
        }

        $rows = TagQuery::GUserRoleListByGId($this->db, $groupIds);
        while (($d = $this->db->fetch_array($rows))){
        ...
        $rows = TagQuery::CategoryList($this->db, $groupIds);
        while (($d = $this->db->fetch_array($rows))){
            $group = $list->Get($d['groupid']);
            if (empty($group)){
                continue;
    ...
    public function OperSave($od){
        ...
        if ($od->id == 0){
            $od->id = TagQuery::OperAppendByObj($this->db, Abricos::$user->id, $od->accountid, $od);
        } else {
            TagQuery::OperUpdateByObj($this->db, $od->id, $od->accountid, $od);
        }
        ...
        TagQuery::AccountUpdateBalance($this->db, $od->accountid);
    ...
    public function OperMoveSave($d){
        ...
        if ($d->id === 0){
            $d->id = TagQuery::OperMoveAppend($this->db, Abricos::$user->id, $d);
        } else {
            $dbMOper = TagQuery::OperMoveInfo($this->db, $d->id);
            if (empty($dbMOper)
            ...
            TagQuery::OperMoveUpdate($this->db, $d->id, $d);
        }

        TagQuery::AccountUpdateBalance($this->db, $d->srcid);
        TagQuery::AccountUpdateBalance($this->db, $d->destid);
    ...
    public function GroupRemove($groupid){
        ...
        TagQuery::GroupRemove($this->db, $groupid);
    ...
    private function GroupAppendMethod($d){
        $groupid = TagQuery::GroupAppend($this->db, Abricos::$user->id, $d);

        // добавление ролей
        foreach ($d->roles as $r){
            TagQuery::GUserRoleAppend($this->db, $groupid, $r->u, $r->r);
        }
    ...
    private function GroupUpdateMethod($d){
        ...
        TagQuery::GroupUpdate($this->db, $groupid, $d);

        foreach ($d->roles as $role){
            $sRole = $group->roles->Get($role->u);
            if (empty($sRole)){
                TagQuery::GUserRoleAppend($this->db, $groupid, $role->u, $role->r);
            } else {
                TagQuery::GUserRoleUpdate($this->db, $groupid, $role->u, $role->r);
            }
        }
        for ($i = 0; $i < $group->roles->Count(); $i++){
            ...
            if (!$find){
                TagQuery::GUserRoleRemove($this->db, $groupid, $sRole->id);
            }
        }
    ...
    private function AccountAppendMethod($groupid, $d){
        ...
        $d->id = TagQuery::AccountAppend($this->db, $userid, $groupid, $d);

        foreach ($d->roles as $r){
            TagQuery::AUserRoleAppend($this->db, $d->id, $r->u, $r->r);
        }

        TagQuery::AccountUpdateBalance($this->db, $d->id);
    ...
    private function AccountUpdateMethod($d){
        ...
        TagQuery::AccountUpdate($this->db, $accountid, $d);

        // обновление ролей доступа к аккаунтам
        foreach ($d->roles as $role){
            $sRole = $account->roles->Get($role->u);
            if (empty($sRole)){
                TagQuery::AUserRoleAppend($this->db, $accountid, $role->u, $role->r);
            } else {
                TagQuery::AUserRoleUpdate($this->db, $accountid, $role->u, $role->r);
            }
        }
        ...
        for ($i = 0; $i < $account->roles->Count(); $i++){
            ...
            if (!$find){
                TagQuery::AUserRoleRemove($this->db, $accountid, $sRole->id);
            }
        }
        TagQuery::AccountUpdateBalance($this->db, $accountid);
    }
    ...
    public function AccountRemove($d){
        ...
        TagQuery::AccountRemove($this->db, $accountid);
    ...
    public function UserList(){
        ...
        $rows = TagQuery::UserListByIds($this->db, $userIds);
        while (($d = $this->db->fetch_array($rows))){
    ...
    private function CategoryAppendMethod($groupid, $title, $isExpense, $parentid = 0, $order = 0){
        ...
        return TagQuery::CategoryAppend($this->db, Abricos::$user->id, $groupid, $title, $isExpense, $parentid, $order);
    }
    ...
    public function CategorySave($d){
        ...
        if ($d->id === 0){
            $d->id = TagQuery::CategoryAppend($this->db, Abricos::$user->id,
                $d->groupid, $d->title, $d->isexpense, $d->parentid, 0);
        } else {
            TagQuery::CategoryUpdate($this->db, $d->groupid, $d->id, $d->title);
        }
    ...
    private function CategoryRemoveMethod($group, $categoryid){
        ...
        TagQuery::CategoryRemove($this->db, $group->id, $categoryid);
        TagQuery::OperRemoveByCategoryId($this->db, $categoryid);
    }
    ...
    public function CategoryRemove($d){
        ...
        $accountList = $this->AccountList();
        for ($i = 0; $i < $accountList->Count(); $i++){
            TagQuery::AccountUpdateBalance($this->db, $accountList->GetByIndex($i)->id);
        }
    ...
    public function OperList($config){
        ...
        $rows = TagQuery::OperListByAIds($this->db, $aids, $fromdt, $enddt, $upddate);
        while (($d = $this->db->fetch_array($rows))){
    ...
    public function OperMoveList($config){
        ...
        $rows = TagQuery::OperMoveListByAIds($this->db, $aids, $fromdt, $enddt, $upddate);
        while (($d = $this->db->fetch_array($rows))){
    ...
    public function OperRemove($operid){
        ...
        $dbOper = TagQuery::OperInfo($this->db, $operid);
        if (empty($dbOper)){
            return;
        }
        if (intval($dbOper['methodid']) > 0){
            $methodid = intval($dbOper['methodid']);
            $dbMOper = TagQuery::OperMoveInfo($this->db, $dbOper['methodid']);
            ...
            TagQuery::OperMoveRemove($this->db, $methodid);
            TagQuery::AccountUpdateBalance($this->db, $srcAccount->id);
            TagQuery::AccountUpdateBalance($this->db, $destAccount->id);
        } else {
            $account = $this->AccountList()->Get($dbOper['accountid']);
            if (empty($account) || !$account->IsWriteRole()){
                return 403;
            }
            TagQuery::OperRemove($this->db, $operid, $account->id);
            TagQuery::AccountUpdateBalance($this->db, $account->id);
        }
