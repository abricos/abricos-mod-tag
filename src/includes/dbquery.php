...
class TagQuery {
    ...
    public static function GroupById(Ab_Database $db, $groupid, $userid){
        $rows = TagQuery::GroupListByIds($db, $groupid, $userid);
        return $db->fetch_array($rows);
    }

    public static function Account(Ab_Database $db, $userid, $accountid){
        $rows = TagQuery::AccountList($db, $userid, 0, $accountid);
        return $db->fetch_array($rows);
    }
    ...
    public static function AccountUpdateBalance(Ab_Database $db, $accountid){
        $row = TagQuery::OperSum($db, $accountid);
    ...
    public static function OperAppendByObj(Ab_Database $db, $userid, $accountid, $od){
        return TagQuery::OperAppend($db, $userid, $accountid, $od->isexpense, $od->value, $od->date, $od->categoryid, $od->descript);
    }
    ...
    public static function OperUpdateByObj(Ab_Database $db, $operid, $accountid, $od){
        return TagQuery::OperUpdate($db, $operid, $accountid, $od->value, $od->date, $od->categoryid, $od->descript);
    }
    ...
    public static function OperMoveAppend(Ab_Database $db, $userid, $od){
        ...
        // расход
        $fromOperId = TagQuery::OperAppend($db, $userid, $od->srcid, true, $od->value, $od->date, 0, $od->descript, $methodid);

        // доход
        $toOperId = TagQuery::OperAppend($db, $userid, $od->destid, false, $od->value, $od->date, 0, $od->descript, $methodid);
    ...
    public static function OperMoveUpdate(Ab_Database $db, $methodid, $od){
        ...
        $dbMOper = TagQuery::OperMoveInfo($db, $methodid);
        TagQuery::OperUpdate($db, $dbMOper['fromoperid'], $dbMOper['fromaccountid'], $od->value, $od->date, 0, $od->descript);
        TagQuery::OperUpdate($db, $dbMOper['tooperid'], $dbMOper['toaccountid'], $od->value, $od->date, 0, $od->descript);
    }

    public static function OperMoveRemove(Ab_Database $db, $methodid){
		$dbMOper = TagQuery::OperMoveInfo($db, $methodid);
		TagQuery::OperRemove($db, $dbMOper['fromoperid'], $dbMOper['fromaccountid']);
		TagQuery::OperRemove($db, $dbMOper['tooperid'], $dbMOper['toaccountid']);
        $sql = "
